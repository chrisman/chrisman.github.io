...
So what I tried next is probably insane, but I remembered that you can use marks in a range[^marks], and you can get line numbers from the expression register[^expr]. So what I did was write a new macro that does this:
...
Go to the top of the block and `@a`
...
[^marks]: You can set some arbitrary marks, and then use them in a range to, for example, search and replace between those two marks: `:'a,'bs/hot dogs/hamburgers`

[^expr]: See `:help line`. You can do something like `:%s/^/\=line(".")/` to number every line in a file for example.
