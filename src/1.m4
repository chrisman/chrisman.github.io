...
class
:   In "the map is not the terrain" fashion, a class is not an object, but is a
    description of an object that can be used to create an object.
:   ...except for in JavaScript, where a class is just syntactic sugar for a
    plain old JavaScript object, providing some convenient access to its
    prototype.

FP
:   Functional Programming: a style of programming that avoids side effects and
    ...
    endofunctors, what's the problem?" and, ultimately, to Lisp.

Factory
:   That thing which facilitates the churning out of products

Mixin
:   When you go to Cold Stone and you have a small number of base ice creams,
...
OOP
:   Object Oriented Programing: a maligned practice of programming that
    requires the  memorization of a dozen  or more Design Patterns  to be
    consistantly useful.
